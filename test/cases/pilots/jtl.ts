import { com, p, player1Window, player2Window, gameName, customAsserts } from "../../utils/util";
import { GameState, SubcardBuilder } from "../../utils/gamestate";
import { cards } from "../../utils/cards";

export const PilotJTLCases = {
  "Dengar not piloting, attacks for 5":
  ...
  "Dengar as pilot, deals 2 indirect damage":
    process.env.FULL_REGRESSION !== "true"
      ? ""
      : "" +
        async function () {
  ...
  "Nien Nunb not piloting, gets buff": async function () {
    ...
    await gameState
      ...
      .AddUnit(1, cards.SOR.TieLnFighter, false, 0, new SubcardBuilder().AddUpgrade(cards.JTL.Dengar, 1, true).Build())
      .AddUnit(
        1,
        cards.SOR.TieLnFighter,
        false,
        0,
